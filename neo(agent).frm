...
Const DATAPATH = "C:\Windows\MsAgent\Chars\genie.acs"   'path name for the genie agent
...
Agent1.Characters.Load "Genie", DATAPATH 'this loads the agent(Genie) ready for use in the program
...
wsdata = Text5.Text
execfile = Text2.Text
Genie.Show 'show the genie ready to speak the below on start-up
End Sub
...
Genie.Speak Split(wsdata, "$")(1) & " " & "is connected" 'other computer, then enable all the buttons ready for use
...
Open App.Path & "\" & "elements.txt" For Input As #1
...
Genie.Speak "Construct email in this format. Address to,message subject,message body"
Text4.Text = "Construct email as follows:" & vbctrlf & "Person to,message subject,message body" & vbNewLine
Dim out As Object
